...
const make_ordered_dictionary_1 = require("./make-ordered-dictionary");
...
exports.waLabelAssociationKey = {
    key: (la) => la.type === LabelAssociationType_1.LabelAssociationType.Chat
...
exports.default = (config) => {
    ...
    const bind = (ev) => {
        ...
        ev.on('messaging-history.set', ({ chats: newChats, contacts: newContacts, messages: newMessages, isLatest, syncType, }) => {
            if (syncType === esm_1.default.b.proto.HistorySync.HistorySyncType.ON_DEMAND) {
                return;
            }
            if (isLatest) {
                chats.clear();
            ...
            for (const update of updates) {
                ...
                if (contacts[update.id]) {
                    ...
                    const contactHashes = await Promise.all(Object.keys(contacts).map(async (contactId) => {
                        const { user } = esm_1.default.b.jidDecode(contactId);
                        return [
                            contactId,
                            esm_1.default.b
                    ...
                    contact =
                        contacts[((_a = contactHashes.find(([, b]) => b === update.id)) === null || _a === void 0 ? void 0 : _a[0]) || ''];
                }
                if (contact) {
                    if (update.imgUrl === 'changed') {
                        contact.imgUrl = socket
                            ? await (socket === null || socket === void 0 ? void 0 : socket.profilePictureUrl(contact.id))
                            : undefined;
                    }
        ...
        ev.on('messages.upsert', ({ messages: newMessages, type }) => {
            switch (type) {
                ...
                case 'notify':
                    for (const msg of newMessages) {
                        const jid = esm_1.default.b.jidNormalizedUser(msg.key.remoteJid);
                        const list = assertMessageList(jid);
                        ...
                        if (type === 'notify' && !chats.get(jid)) {
                            ev.emit('chats.upsert', [
                                {
                                    id: jid,
                                    conversationTimestamp: lodash.toNumber(msg.messageTimestamp),
                                    unreadCount: 1,
                                },
                            ]);
                        }
        ...
        ev.on('messages.update', (updates) => {
            ...
            for (const { update, key } of updates) {
                const list = assertMessageList(esm_1.default.b.jidNormalizedUser(key.remoteJid));
            ...
            if (metadata) {
                switch (action) {
                    ...
                    case 'promote':
                        ...
                        break;
                    case 'remove':
                        metadata.participants = metadata.participants.filter((participant) => !participants.find((p) => p.id === participant.id));
                        break;
                }
            }
        });
    ...
    const fromJSON = (json) => {
        ...
        for (const jid in json.messages) {
            ...
            for (const msg of json.messages[jid]) {
                list.upsert(esm_1.default.b.proto.WebMessageInfo.create(msg), 'append');
            }
        }
    };
    return {
